...
function saveMapping(map: Record<string, any>) {
  fs.writeFileSync(MAPPING_FILE, JSON.stringify(map, null, 2), "utf-8");
}

async function postWithRetry(url: string, body: any, headers: Record<string, string>, maxAttempts = 3) {
  let attempt = 0;
...
async function createRecord(appId: number, record: any, dryRun = false, bearerToken?: string) {
...
function mapFlowToRecord(flow: any) {
  return {
...
function mapBlockToRecord(block: any, flowId: string) {
  return {
...
function mapEdgeToRecord(edge: any, flowId: string) {
  return {
...
app.post("/import", upload.single("file"), async (req, res) => {
  try {
    ...
    if (!bearerToken) {
      ...
      if (userId) {
        try {
          ...
          if (!tokenInfo) throw new Error("no token info for user");
          ...
          console.warn(`token load/refresh failed for user ${userId}:`, (e as any)?.message || String(e));
        }
      }
    }
    ...
    const flowResp = await createRecord(flowsAppId, flowRecord, dryRun as boolean, bearerToken);
    const createdFlowId = flowResp?.id || null;
    if (!dryRun) mapping[`flow:${flow.id}`] = { kintoneId: createdFlowId };

    // Create Blocks
    for (const b of blocks) {
      const rec = mapBlockToRecord(b, flow.id);
      const r = await createRecord(blocksAppId, rec, dryRun as boolean, bearerToken);
      if (!dryRun) mapping[`block:${b.id}`] = { kintoneId: r?.id };
    }

    // Create Edges
    for (const e of edges) {
      const rec = mapEdgeToRecord(e, flow.id);
      const r = await createRecord(edgesAppId, rec, dryRun as boolean, bearerToken);
      if (!dryRun) mapping[`edge:${e.id}`] = { kintoneId: r?.id };
    }
    ...
    for (const it of iterations) {
      ...
      const r = await createRecord(iterationsAppId, rec, dryRun as boolean, bearerToken);
      if (!dryRun) mapping[`iteration:${it.id}`] = { kintoneId: r?.id };
    }

    // Persist mapping
    if (!dryRun) saveMapping(mapping);
...
app.post("/auth/exchange", async (req, res) => {
  try {
    ...
    if (!tokenEndpoint || !cid || !csec || !ruri) return res.status(400).json({ error: "OAuth config missing (tokenUrl/clientId/clientSecret/redirectUri)" });
    ...
    const resp = await axios.post(tokenEndpoint, params.toString(), { headers: { "Content-Type": "application/x-www-form-urlencoded" } });
...
async function refreshTokenForUser(userId: string) {
  const map = loadMapping();
  const tokens = (map.oauthTokens || {})[userId];
  if (!tokens || !tokens.refreshToken) throw new Error("no refresh token stored for user");
  ...
  if (!tokenEndpoint || !clientId || !clientSecret) throw new Error("OAuth config missing for refresh");
  ...
  if (body.refresh_token) map.oauthTokens[userId].refreshToken = body.refresh_token;
