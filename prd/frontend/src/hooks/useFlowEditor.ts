...
import type { Block, BlockType, Edge, Flow, Iteration, Suggestion } from "../models/types";
import { api } from "../services/api";
import { getDefaultMetaByType } from "../utils/analysisRules";
import type { FlowBundle } from "../models/types";
...
export function useFlowEditor() {
  ...
  const connectBlocks = useCallback(
    async (fromBlockId: string, toBlockId: string, label?: string) => {
      if (!flow) return;
  ...
  const importJson = useCallback(
    async (file: File) => {
      ...
      const bundle = JSON.parse(text) as FlowBundle;
