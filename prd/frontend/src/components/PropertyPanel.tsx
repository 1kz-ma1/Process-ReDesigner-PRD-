...
import type { NodeData, ValidationMessage } from "../models/types";
import { getLabel } from "../utils/i18n";

interface PropertyPanelProps {
  node: NodeData | null;
  onUpdateNode: (patch: Partial<NodeData>) => void;
  inlineSuggestions?: ValidationMessage[];
  onApplySuggestion?: (msg: ValidationMessage, fixId?: string) => void;
}
      ...
      {node && (
        <div className="form-group">
          {inlineSuggestions.length > 0 && (
            <section className="inline-suggestions" aria-label="選択ノードの提案">
              <h3>ミニ提案</h3>
              {inlineSuggestions.slice(0, 3).map((msg) => {
                const firstFix = msg.fixes?.[0];
                return (
                  <article key={msg.id ?? msg.message} className={`inline-suggestion sev-${msg.type}`}>
                    <p>{msg.message}</p>
                    {firstFix && onApplySuggestion && (
                      <button type="button" onClick={() => onApplySuggestion(msg, firstFix.id)}>
                        適用
                      </button>
                    )}
                  </article>
