 ...
 * 存在しないノードを指すエッジが無い
 */
function checkReferenceIntegrity(doc) {
    const messages = [];
 ...
 */
function checkConditionEdges(doc) {
    const messages = [];
    // condition ノードを列挙
 ...
 * 接続されていないノード（接続なし）
 */
function checkIsolatedNodes(doc) {
    const messages = [];
 ...
 */
export function formatValidationMessages(messages) {
