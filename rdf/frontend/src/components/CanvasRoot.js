...
export default function CanvasRoot({ doc, selectedNodeId, onSelectNode, onViewportChange, onAddNode, onUpdateNode, onDeleteNode, onAddEdge, onUpdateEdge, onDeleteEdge, }) {
    const svgRef = useRef(null);
    const [isDragging, setIsDragging] = useState(false);
    const [dragStart, setDragStart] = useState(null);
    const [connectMode, setConnectMode] = useState(null); // 接続モード中の送信元ノード
    const [loopMode, setLoopMode] = useState(null); // ループ接続モード中の送信元ノード
    const [miniMenuPos, setMiniMenuPos] = useState(null); // 小メニューの位置
    const [miniMenuNodeId, setMiniMenuNodeId] = useState(null); // 小メニューの対象ノード
    // ビューポート状態（doc から読む）
    ...
    const handleCreateBranch = useCallback((sourceId) => {
    ...
    const handleDuplicateNode = useCallback((sourceId) => {
        const source = doc.nodes.find((node) => node.id === sourceId);
        if (!source) {
            return;
        }
        const duplicateId = onAddNode(source.type, `${source.name}_copy`, source.position.x + 48, source.position.y + 48, structuredClone(source.meta));
        onSelectNode(duplicateId);
    }, [doc.nodes, onAddNode, onSelectNode]);
    const handleFlipNode = useCallback((targetId) => {
    ...
    return (_jsxs("div", { className: "canvas-root", children: [_jsxs("svg", { ref: svgRef, className: "canvas-svg", onWheel: handleWheel, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseUp, onClick: handleCanvasClick, onContextMenu: (e) => e.preventDefault(), children: [_jsx("defs", { children: _jsx("pattern", { id: "gridPattern", width: "50", height: "50", patternUnits: "userSpaceOnUse", children: _jsx("path", { d: "M 50 0 L 0 0 0 50", fill: "none", stroke: "#e5e7eb", strokeWidth: "0.5" }) }) }), _jsx("rect", { width: "10000", height: "10000", fill: "url(#gridPattern)" }), _jsxs("g", { transform: `translate(${viewport.x}, ${viewport.y}) scale(${viewport.zoom})`, children: [doc.edges.map((edge) => (_jsx(EdgeView, { edge: edge, nodes: doc.nodes, isSelected: selectedNodeId === edge.source || selectedNodeId === edge.target, onDelete: () => onDeleteEdge(edge.id) }, edge.id))), doc.nodes.map((node) => (_jsx(NodeView, { node: node, isSelected: selectedNodeId === node.id, isConnectSource: connectMode === node.id, isLoopTarget: loopMode !== null && parentCandidates.has(node.id), onSelect: () => handleSelectNode(node.id), onContextMenu: (e) => handleContextMenu(e, node.id), onDragMove: (dx, dy) => {
                                    onUpdateNode(node.id, {
                                        position: { x: node.position.x + dx, y: node.position.y + dy },
                                    });
                                } }, node.id)))] })] }), miniMenuPos && miniMenuNodeId && (_jsx(MiniMenu, { position: miniMenuPos, nodeId: miniMenuNodeId, onConnect: () => {
